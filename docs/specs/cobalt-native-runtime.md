...
Framerail uses four native media queries sharing `$small-screen-max: 850px`; typography, layout, and toast rules retain their inclusive 850px boundary. The `include-media` package is no longer imported by Framerail styles. Final gate `/tmp/cobalt-media-native-final-gate.json` passes with boundaries at `b4abf56f4`: two Sass tests report no deprecations; compiled `main.css` and `toasts.css` are byte-identical to their prior outputs (respectively `86052b5908e905b550f183c1942b3638c333a020f5ac94e47b5f1bac3d16682f` and `822cb6eb5f17e3eaee0d3c203a3a04ff3844eb97a9c588667cbeb8ccdf3395d2`); Chromium computed styles match at 849, 850, and 851px; Stylelint, Prettier, and the new test's ESLint pass; and `svelte-check` reports 0 errors and 0 warnings with the required pnpm/Node PATH, Sass, and CSRF output clean. At `2a6ee1365`, the two `Toasts.svelte` event parameters changed from `any` to Svelte's `CustomEvent<null>` contract; final gate `/tmp/cobalt-toast-types-final-gate.json` proves identical preprocessed, generated JavaScript, and CSS hashes. Direct ESLint, Prettier, and `svelte-check` each exit 0 with clean output and 0 diagnostics. This does not establish the pnpm wrapper as operational: its attempts fail before install. The direct dependency declaration is temporarily retained because isolated pnpm removal rewrites unrelated peer resolutions; remove that metadata when a bounded lockfile update is available. Shared installed dependencies and legacy `web/resources` styles are unchanged.
