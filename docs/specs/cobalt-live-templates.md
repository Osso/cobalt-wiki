...
Tokenizer profiling (2026-09-24): an actual Atley preview exceeded the unchanged 2.5-second aggregate deadline at 2,938 ms; its 134,855-byte preprocessed input produced 51,987 tokens and spent ~1,338 ms in lexing, versus ~229 ms in SQL. A standalone exact-grammar benchmark measured a faster whitespace-first token order with identical token rules, slices, and UTF-8 byte spans. This is a tokenizer measurement, not proof that the preview now meets its deadline; the configured limits remain unchanged.
