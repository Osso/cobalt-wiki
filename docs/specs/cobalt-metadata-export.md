...
`tests/cobalt_migration/test_metadata_export.py`: 11 targeted synthetic tests, including two distinct pages, a failing second page followed by resume, exact metadata fields, mismatched inputs, denied/missing outcomes, retry delays and atomic-write failure.
