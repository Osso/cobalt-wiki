...
- [x] Normalize bare `@@` block-mapping tokens and historically accepted plain block-mapping values ending in a colon (including `orc: Orc:`), preserving the literal label, quoted strings, comments, block text, and line endings. Do not repair unrelated malformed YAML or flow syntax.
...
This supersedes the former claim that historical parser behavior was unavailable or unproven. It establishes acceptance by that pinned legacy parser only—not behavior of the current hosted implementation. Approved standalone parser compatibility accepts `orc: Orc:` as code `orc`, literal label `Orc:`; `deepwell/wikidot-forms/tests/compatibility.rs` asserts that output and adjacent preservation/rejection boundaries. The previous local NPC route returned schema RPC 4000 and creation HTTP 500; this change has not been deployed or tested through that route. The source inventory contains no saved NPC records. No archive rewrite is authorized; hosted NPC behavior remains unproven.
