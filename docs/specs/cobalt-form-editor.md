...
- `deepwell/tests/editor_pages.rs`: native local endpoint coverage for case-insensitive literal prefixes, every-ASCII-space normalization, literal `%`/`_`/`\\` handling, slug order, current nondeleted titles, trusted site/user context, View filtering before the 20-result cap, and query bounds. Author-run proof passed 5/5 at `2e0aab1`; output was captured but no artifact was saved.
- `framerail/tests/local/editor-wizards.mjs`: authenticated local browser acceptance passed 1/1 after `2e0aab1` (`/tmp/claude/cobalt-editor-pages-browser.log`): prefix/sort page suggestions, all six inserts and Preview, empty/nonempty attachment lookup, loaded selected attachment and rendered image, Flickr invalid/numeric input, missing-target attachment exclusion, cancel focus/selection restoration, unchanged source/revisions, and missing target still absent. Separate attachment browser proof remains retained.
...
- [ ] Confirm exact hosted-server autocomplete behavior beyond the published PHP/current-client contract. Local dedicated autocomplete is implemented and proven: case-insensitive literal slug prefix after every-ASCII-space normalization, slug order, current nondeleted titles, trusted site/user context, View filtering before a local 20-result cap, native author 5/5 without a saved artifact, and browser 1/1 at `/tmp/claude/cobalt-editor-pages-browser.log`. The published PHP source is unbounded; local cap parity is not claimed.
