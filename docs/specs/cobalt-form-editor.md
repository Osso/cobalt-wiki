...
- [ ] Replace reused full-text page search with a dedicated page-link lookup. Source evidence requires a case-insensitive slug-prefix match after replacing every ASCII space with `-`, slug ordering, and no `title=yes` request. The in-progress local endpoint will apply permissions and return at most 20 visible matches; its trusted context uses the current revision title. Backend proof is not yet complete.
...
- URL defaults to `http://`, has optional anchor text, and defaults the new-window checkbox off. Page link takes page name plus optional anchor; the hosted JavaScript begins at two characters after a 0.5-second delay and requests `PageLookupQModule` with only `s`, `module`, and `q`. The PHP module replaces every ASCII space with `-`, performs a case-insensitive `unix_name` prefix match, and orders by `unix_name`; title matching occurs only if `title=yes`, which this wizard does not send. The local full-text reuse is being replaced by a permission-filtered dedicated slug-prefix lookup capped at 20 visible results and using the trusted current revision title. Its backend implementation and proof remain incomplete; this is not source autocomplete parity.
...
- [ ] Complete and prove dedicated page-link autocomplete. Source behavior is slug-prefix lookup, case-insensitive with every ASCII space changed to `-`, slug-sorted, and no title matching because the wizard omits `title=yes`. The local bounded, permission-filtered 20-result contract and trusted current-revision-title context are in progress; backend work and browser/native proof remain open.
