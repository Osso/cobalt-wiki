...
Do **not** claim every original native revision is byte-identical. The initial native-revision fingerprint intentionally excluded compiled outputs but mistakenly included `updated_at`, which native rerendering mutates. Of 61 early changed fingerprints, 18 reconstruct exactly when `updated_at` is normalized to null; the other 43 original full hashes cannot be reconstructed. Exhaustive bounded artifact search under `/home/osso/.local/share/cobalt-wiki` found no prior full-row timestamps for those 43; do not claim reconstruction. Original page/file rows remain identical, and normalized snapshots of source content and metadata from the 4,055-page checkpoint onward match. Later original timestamp changes are acceptable only while that final normalized baseline remains stable.
