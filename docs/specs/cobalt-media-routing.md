...
- [ ] Native gallery viewing (`cef28e93b`, `01e1bfaa4`, formatting/types `28b30c9cc`) is independently verified locally, not deployed publicly. Component/browser tests passed 3/3 (`/tmp/claude/cobalt-badges-node-tests.log`); main inspected `/tmp/claude/cobalt-badges-viewer-fixed.png`. Follow-up checks found no gallery lint/style/type errors, and transient browser probes confirmed modifier handling, isolated gallery groups and visible image-load errors. Five existing imported-history type errors remain outside this scope. Production still sends an ordinary Badges thumbnail click to the homepage.
