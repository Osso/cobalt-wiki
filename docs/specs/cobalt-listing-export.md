...
Main accepted authenticated `/pagelist/p/277` through the browser transport: HTTP 200, 73 names, highest page 277. A protected complete run recorded 277/277 pages and 6,092 unique literal fullnames in `listing-checkpoint.json`. This proves the listing-export transport/run only; reconciliation against the native archive remains pending. No archive-name inference, target import, or deployment is provided.
