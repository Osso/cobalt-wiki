...
- [x] Submit and reconcile the acquired 240-record homepage history set (`0`–`239`) on main local site `6000000` through `import_wikidot_history`. The protected preflight bound `home:start` page `3000000141`, source page `1310927108`, guarded current revision `3000000281`, and a zero-row starting inventory; the response reported `{"inserted":240}`. Readback returned `100/100/40` rows with all 240 metadata records matching the exact 11 required keys, all 240 display-decoded bodies, raw-HTML hashes, and acquired timestamps. Preservation fingerprints retained 6,116 pages, 10,092 native revisions, zero drafts, and nine unrelated imported-history records; homepage history changed `0→240` and queue depth remained `0`. Independent verification remains pending in the [replica status](../wiki/systems/cobalt-replica-status.md#imported-source-history).
...
- [x] Submit and read back the acquired set on main local backend `2749` / site `6000000`; the protected response reported 240 inserted records. The preflight current-body identity was 2,473 bytes and SHA-256 `eb0478369cf1cae46acb85b994c77162e3116ce7694ca7c5399ae26e46890beb`, matching the protected main plan; guarded revision `3000000281`, source/target payload files, and preserved-state fingerprints remained unchanged. Main browser proof passed 1/1 in 27 seconds using fresh `3090` context without credentials or a Basic challenge: five pages read 240 metadata rows, sources were `239/0`, provenance/read-only/no-rollback behavior remained, current render hash and reload passed, and only three read POST actions occurred. Independent verification remains pending; see the [replica status](../wiki/systems/cobalt-replica-status.md#imported-source-history).
