...
- [ ] Continue authenticated source acquisition only for the 933 known denied bodies on 68 pages after the homepage: anonymous transport is live-proven, but its `no_permission` responses are explicit gaps, not universal inaccessibility. Keep the two unresolved source identities separate (redirect position `3`; denied position `621`); do not fabricate them. The largest retained page has 14,091,755 decoded bytes, so plan bounded bulk requests before implementation.
