...
- [x] Read the current local pilot API on backend `2748`: three cursor pages (`100/100/40`) returned 240 rows, each with exactly the 11 required metadata keys; timestamps compared as instant-equivalent `Z`/`+00:00`, and revisions 239 and 0 matched retained display-decoded bodies. The retained body proof was reused; the exact-key capture closes the prior returned-key proof gap. Current proof is recorded in the [replica status](../wiki/systems/cobalt-replica-status.md#imported-source-history).
...
- [x] Install the pilot-only user-service override at `/home/osso/.config/systemd/user/cobalt-local-framerail.service.d/10-history-pilot.conf`, selecting `vite.history-pilot.config.ts` on `5173`, and retain a GREEN browser proof at `2113c00b1`: 240 metadata rows across five pages, 239/0 body hashes, read-only/provenance/no-rollback behavior, allowlisted native and imported-history reads only, current-body hash and reload. The prior `0db5c2fc0` hydration failure and post-isolation selector correction `49f789a64` are historical; `2113c00b1` corrects an erroneous block on native history reads. This proves the isolated pilot, not main/production parity; verifier `681` remains pending. See the [replica status](../wiki/systems/cobalt-replica-status.md#imported-source-history).
