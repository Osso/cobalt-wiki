...
Cobalt history import must store authorized source revision history without presenting the latest-source migration as complete history. The current migration has 6,092 pages and 1,471 files. The retained source-history inventory has 6,090 accepted source identities, 45,369 listed revisions, and 44,196 display-decoded bodies. Main-local bulk application `26bf5ef1e` produced 44,436 imported-history records across 6,022 pages: 44,187 new records and 249 preserved existing records. Offline verifier `707` passed identity, body, core-metadata, newly added raw-HTML/acquired-field, retained Isabeau-9 capture-provenance, and authenticated 240-homepage-payload reconciliation; protected evidence is `/home/osso/.local/share/cobalt-wiki/local-full/history-bulk/independent-data-verification.json`. The 1,173 source-unavailable records include the 240 already-known homepage records; 933 denied bodies remain across 68 pages, with two metadata-unresolved source identities. Import-phase before/after evidence preserves 6,116 pages, 10,092 native revisions, zero drafts, 249 existing imported-history records, and zero jobs. Later UI reads upgraded renderer caches for two selected native revisions; that bounded exception is not history mutation and does not permit a claim that all native full-row fingerprints stayed unchanged after UI. Final retained-proof reconciliation `708` remains pending; this is not full-goal completion. No new source acquisition, production import, or intentional mutation of native/current/draft records occurred. The user approved importing historical text from Wikidot's display representation even where tabs may become spaces; raw responses and that limit must remain recorded. Historical title/slug/tags may be null and 43 original full-row fingerprints cannot be reconstructed; neither limit permits a byte-exact historical representation claim. See the [current POC import contract](cobalt-poc-import.md), the [native revision import endpoint](../../deepwell/src/endpoints/import.rs), the [native import service](../../deepwell/src/services/import/service.rs), and the [stock WikiComma importer entry point](../../deepwell/importer/__main__.py).
...
- [ ] Continue authenticated source acquisition only for the 933 known denied bodies on 68 pages after the homepage: anonymous transport is live-proven, but its `no_permission` responses are explicit gaps, not universal inaccessibility. Keep the two unresolved source identities separate (redirect position `3`; denied position `621`); do not fabricate them. The largest retained page has 14,091,755 decoded bytes. Main-local missing-only bulk application is complete and reconciled at the data boundary; retained-proof reconciliation `708` remains pending.
...
- [x] Independently reconcile completed missing-only main-local archive application. CLI `26bf5ef1e` processed 6,090 targets and the actual database has 44,436 imported-history records across 6,022 pages: 44,187 new plus 249 preserved. Offline verifier `707` passed all identity/body/core-metadata checks, raw-HTML and acquired fields newly added to the audit, retained Isabeau-9 capture provenance, and the authenticated 240-homepage payload match; `/home/osso/.local/share/cobalt-wiki/local-full/history-bulk/independent-data-verification.json` is the protected root artifact. `/tmp/claude/cobalt-bulk-history-{before,after}.log` shows the import phase preserved all fingerprints for 6,116 pages, 10,092 native revisions, zero drafts, and 249 existing imported-history rows, with zero jobs before and after. Main UI proof passed 2/2 in `/tmp/claude/cobalt-bulk-history-ui-{1312278321,1331497889}.log`: 1,133 revisions across 23 pages and 223 revisions across five pages, including metadata, newest/oldest bodies, read-only/provenance, no unexpected POST, current-render hash, and reload. After UI, only selected native revisions `3000004643` and `3000007537` changed: `updated_at`, `compiled_generator`, and three compiled-HTML hash fields; `/home/osso/.local/share/cobalt-wiki/local-full/history-bulk/post-ui-native-differences.json` and native-field hashes prove all other fields unchanged. This normal read-triggered cache upgrade follows the known renderer-generator mismatch in `deepwell/src/services/page_revision/service.rs:314` and `deepwell/src/services/page_revision/service.rs:896`; it is not history mutation. Do not claim all native full-row fingerprints unchanged after UI. Final retained-proof reconciliation `708` remains pending.
