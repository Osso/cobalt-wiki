...
- [ ] Install the pilot service override selecting that configuration, then run the actual browser flow and retain a GREEN UI proof. The new configuration alone neither switches a runtime nor fixes the UI. `/tmp/claude/cobalt-history-hydration-trace.json` observes initialized `HVRELRUH` operations followed by uninitialized `WCKNV63C` `get_first_child`, but does not establish shared cache as the complete cause before isolation passes. `c7859d0` is historical only; no current UI pass exists. See the [replica status](../wiki/systems/cobalt-replica-status.md#imported-source-history).
