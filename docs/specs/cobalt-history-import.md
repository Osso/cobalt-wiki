...
- [ ] Restore a current local UI history proof. `c7859d0` is historical only; the observed current browser delivery returned `200 text/html` and hydration failed with `TypeError: Cannot read properties of undefined (reading 'call')`, leaving an empty DOM. No current UI pass exists. See the [replica status](../wiki/systems/cobalt-replica-status.md#imported-source-history).
