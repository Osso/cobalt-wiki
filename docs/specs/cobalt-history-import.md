...
Cobalt history import must store authorized source revision history without presenting the latest-source migration as complete history. The current migration has 6,092 pages and 1,471 files. The retained source-history inventory has 6,090 accepted source identities, 45,369 listed revisions, and 44,196 display-decoded bodies; it still has 933 known denied bodies across 68 pages after the authenticated homepage evidence and two metadata-unresolved identities, so source acquisition is not complete. The user approved importing historical text from Wikidot's display representation even where tabs may become spaces; raw responses and that limit must remain recorded. The bounded authenticated homepage import acquired 240 listed revisions and all 240 source-module representations; it does not establish byte-exact historical bodies or whole-site history. See the [current POC import contract](cobalt-poc-import.md), the [native revision import endpoint](../../deepwell/src/endpoints/import.rs), the [native import service](../../deepwell/src/services/import/service.rs), and the [stock WikiComma importer entry point](../../deepwell/importer/__main__.py).
...
- [ ] Apply and independently verify the missing-only main-local archive import. The validated 44,196-body anonymous archive has nine matching existing records, leaving 44,187 expected missing records; this is a planned count, not an applied result.
