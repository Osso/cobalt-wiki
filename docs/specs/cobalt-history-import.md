...
- [ ] Continue authenticated source acquisition only for the 933 known denied bodies on 68 pages after the homepage: anonymous transport is live-proven, but its `no_permission` responses are explicit gaps, not universal inaccessibility. Keep the two unresolved source identities separate (redirect position `3`; denied position `621`); do not fabricate them. The largest retained page has 14,091,755 decoded bytes. The bounded bulk importer is implemented and tested, but no main-local archive application has run.
...
- [ ] Independently verify the completed missing-only main-local archive application. CLI `26bf5ef1e` reported 6,090 target pages processed, 44,187 inserted records, 249 existing records preserved by selection, 933 unresolved bodies, `available_history_applied: true`, and `failed_page_id: null` in `/home/osso/.local/share/cobalt-wiki/local-full/history-bulk/application-{report,summary}.json`. Verify exact post-application database totals (expected 44,436 including 240 homepage and 44,196 anonymous history), preservation snapshots, raw-HTML reconciliation, UI proof, and verifier `705`; until then, do not claim those results.
