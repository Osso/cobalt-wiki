...
Current local gateway proof: authenticated homepage `200`, unauthenticated homepage `401`, theme CSS `200`. This is not full-replica proof: attachments and form fixtures are absent, browser hydration and visual parity remain unproven, and the initial HTML did not yet reference the theme stylesheet. Rebuild the debug Deepwell binary with a targeted backend check before testing backend edits; the current binary is not certified to match `HEAD`. Frontend changes hot-reload through Vite. Next proof: add minimal archived attachment/form fixtures, confirm theme attachment in rendered HTML, then browser-edit a hydrated form, save, reload, and verify the stored value.
