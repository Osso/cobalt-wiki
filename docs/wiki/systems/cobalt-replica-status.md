...
| [Forms](../../specs/cobalt-data-form-schema.md) and rendering | Whole-record schema/value handling, authorized form edits, native DB regressions; all archived sources parse/render without panic. [Automatic link titles](../../specs/cobalt-link-titles.md) now use actual target revisions; four affected cached pages refreshed without new revisions. [Local same-site include expansion](../../specs/cobalt-includes.md) passed six native cases: nested/empty-value body and nav expansion, unavailable/denied/foreign isolation, foreign dependency exclusion, and cycle/depth preservation. | Hydrated form save/reload, ListPages/form token parity, source include grammar/SUO support and local-corpus/browser proof, included-page/permission invalidation, inline-file domain routing and multi-colon reference normalization |
| Production rendering ([live templates](../../specs/cobalt-live-templates.md), [ListPages/CountPages](../../specs/cobalt-list-pages.md), Join) | Deployed 2026-09-23 from `cobalt-rendering`; every page rerendered. Live comparison with Wikidot: see below | ListPages pagination, listing/template/include invalidation, original created/updated dates, NewPage/TagCloud/PagesByTag/SiteChanges/Rate/ManageSite modules, show-to for admins |

## Production rendering (2026-09-23)

`cobalt-rendering` is deployed through Sakuin (`cobaltWiki` input pinned in `flake.lock`; `./ops/deploy.sh`). It adds category form templates, ListPages and CountPages, show-to stripping, Wikidot comment/raw ordering, lenient image and div heads, literal `form_data`, Join markup, and fixes link updates that failed with `page_connection_pkey` when a page's many links changed.
...
- Deploy time: Deepwell builds with crane; dependencies are cached from Cargo manifests only and the Nix build uses thin LTO. A Deepwell code change deployed in 6m12s (previously over 10 minutes).
- Include expansion records no include connections, so editing an included page does not rerender its includers.
- Known render cost: character pages run four ListPages queries (about 20–55 ms each under load, no tag index); three synchronous rerenders hit the 500 ms preprocess timeout while the queue was busy and succeeded on retry.
