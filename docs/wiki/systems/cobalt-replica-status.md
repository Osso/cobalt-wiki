...
| [Forms](../../specs/cobalt-data-form-schema.md) and rendering | Whole-record schema/value handling, trusted create/edit context, native DB regressions, and one authenticated local browser form save/reload. The browser flow changed text/wiki/radio/select values while retaining three untouched schema fields and an unknown `true` field; anonymous UI/action writes were denied. All archived sources parse/render without panic. [Automatic link titles](../../specs/cobalt-link-titles.md) now use actual target revisions; four affected cached pages refreshed without new revisions. [Local same-site include expansion](../../specs/cobalt-includes.md) passed six native cases: nested/empty-value body and nav expansion, unavailable/denied/foreign isolation, foreign dependency exclusion, and cycle/depth preservation. `c7859d0` integrates the user-owned rendering branch locally: ListPages, live templates, show-to handling, and FTML fixes now build locally. | Formatting toolbar, Preview, Save Draft, source rich-editor parity, broader form/browser coverage, local browser validation of homepage/navigation/ListPages, pagination/invalidation, source include grammar/SUO support, included-page/permission invalidation, inline-file domain routing and multi-colon reference normalization |
| [Site search](../../specs/cobalt-search.md) | Backend fake-HTTP proof covers server credential use, visible compiled-text extraction, site/permission revalidation before paging, and Meilisearch document tasks. Framerail server/component tests cover the header form, trusted-context RPC call, result paging, bounds, escaping, and unavailable state. `d07869d` mounts the source header form. | End-to-end index lifecycle: `31b1a9f` commits outbox/worker code, but mutation hooks and backfill are absent; real indexed-page DB proof, header/result browser acceptance, restart behavior, and deployment remain open. |
| Production rendering ([live templates](../../specs/cobalt-live-templates.md), [ListPages/CountPages](../../specs/cobalt-list-pages.md), Join) | Deployed 2026-09-23 from `cobalt-company`; every page rerendered. Text similarity to live Wikidot: atley 0.996, roster 0.928 and writings 0.923 (remaining: pagination, footer, pages created after the snapshot); `stats` totals match except one post-archive RP log | ListPages pagination, listing/template/include invalidation, original created/updated dates, NewPage/TagCloud/PagesByTag/SiteChanges/Rate/ManageSite modules, show-to for admins |
...
Verified 2026-09-23: Meilisearch `1.43.1` is active only at `127.0.0.1:27700`, with a separate private database and a server-only key. `d07869d` mounts the source header form, while `31b1a9f` commits outbox/worker code without mutation lifecycle hooks or backfill. This records neither end-to-end index lifecycle, real indexed-page search correctness, browser acceptance, restart behavior, nor deployment. Those boundaries remain in the [site search spec](../../specs/cobalt-search.md).
