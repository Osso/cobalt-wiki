...
| [Forms](../../specs/cobalt-data-form-schema.md) and rendering | Whole-record schema/value handling, authorized form edits, native DB regressions; all archived sources parse/render without panic. [Automatic link titles](../../specs/cobalt-link-titles.md) now use actual target revisions; four affected cached pages refreshed without new revisions. [Local same-site include expansion](../../specs/cobalt-includes.md) is committed: a nested native fixture substitutes variables while preserving stored source bytes. | Hydrated form save/reload, ListPages/form token parity, actual source include parser coverage and local-corpus/browser proof, inline-file domain routing and multi-colon reference normalization |
