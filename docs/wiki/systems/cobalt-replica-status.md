...
This is not full visual parity. Remaining local browser priority is top navigation tabs and sidebar matching the actual original. The full import remains incomplete; do not infer full counts, start another importer, or deploy. See [same-site media routing](../../specs/cobalt-media-routing.md) for the rendering contract.
