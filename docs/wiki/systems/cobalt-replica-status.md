...
Observed 2026-09-23; independent browser proof is pending and this section is not verified. The full preview is `127.0.0.1:3090` (Framerail `5174`, Deepwell `2749`, database `cobalt_local_full`). Missing images had two local causes: the database had no attachments and the gateway omitted `ALL` file routing, so image URLs returned HTML with status `200`. The earlier comparison also used `home:start`; the public root is `home:_public` (metadata ID `1312457213`), now the local default.

The local nginx configuration now mirrors the production file-route regex from `install/nixos/poc-gateway.nix`, retains Basic authentication, no-index, and trusted-site headers, and routes to WWS at `127.0.0.1:3470`. WWS started with its actual `S3_FILES_BUCKET` and `S3_TEXT_BLOCKS_BUCKET` contract; `.env.example`'s old `S3_BUCKET` and `ADDRESS` names are not the runtime contract. Immutable full-plan checks loaded 505 original image/icon assets; the priority script reached its file loop end, and the unnecessary rerender step was removed without source changes. Direct HTTP checks returned `200` with correct MIME types for the main logo PNG, background JPEG, and placeholder JPEG.

The full import remains incomplete under external PID `214502`; do not start another importer or deploy. User priority is images first; the user judges readiness and no deployment is authorized without approval. See [same-site media routing](../../specs/cobalt-media-routing.md) for the rendering contract.
