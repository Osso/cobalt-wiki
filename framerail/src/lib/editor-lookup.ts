...
async function postLookupAction(action: string, body = new FormData()) {
...
export async function lookupEditorPages(
  ...
  const result = await postLookupAction("editorPages", body)
...
export async function lookupEditorAttachments(
  ...
  const result = await postLookupAction("editorAttachments")
